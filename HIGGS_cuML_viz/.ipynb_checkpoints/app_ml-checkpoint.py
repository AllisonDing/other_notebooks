...
df = pd.read_csv("HIGGS.csv", names=colnames, nrows = 1_000_000)  # Subsample 100k
print('Checking df type: ', df.info())
...
print('Checking df_gpu type: ')
df_gpu.info()

# Perform K-Means Clustering using cuML
kmeans = KMeans(n_clusters=5, random_state=42, n_init = 10)
kmeans.fit(df_gpu)
print('Checking KMeans: ', type(KMeans))
print('Checking UMAP: ', type(UMAP))
...
print('embedding_3d is: ', type(embedding_3d))
...
def update_figure(selected_cluster):
    ...
    if selected_cluster == "all":
        df2d = plot_df_2d
        df3d = plot_df_3d
        print('Checking dataframe type inside update_figure function: ')
        df2d.info()
    else:
